...
void quantize_row_turbo4_0_ref(const float * GGML_RESTRICT x, block_turbo4_0 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int block = 0; block < nb; block++) {
        ...
        /* Step 5: Residual in rotated domain (simplified QJL — no projection matrix) */
        float residual_norm_sq = 0.0f;
        for (int i = 0; i < d; i++) {
            float r = rotated[i] - CENTROIDS_3BIT[indices[i]];
            residual_norm_sq += r * r;
        }
        float residual_norm = sqrtf(residual_norm_sq);

        /* Step 6: Pack — 2+1 split (same layout as turbo3) */
        y[block].norm  = GGML_FP32_TO_FP16(corrected_norm);
        y[block].rnorm = GGML_FP32_TO_FP16(residual_norm);
        ...
        for (int i = 0; i < d; i++) {
            ...
            /* Lower 2 bits → qs (4 per byte) */
            y[block].qs[i / 4] |= (uint8_t)((idx & 0x3) << ((i % 4) * 2));
            /* Upper 1 bit → qh (8 per byte) */
            if (idx & 0x4) {
                y[block].qh[i / 8] |= (1 << (i % 8));
            }
            /* QJL: sign of residual → signs (8 per byte) */
            float r = rotated[i] - CENTROIDS_3BIT[idx];
            if (r >= 0.0f) {
...
void dequantize_row_turbo4_0(const block_turbo4_0 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    /* 3-bit PolarQuant + QJL: 2+1 split unpack, stays in rotated domain */
    for (int block = 0; block < nb; block++) {
        float norm  = GGML_FP16_TO_FP32(x[block].norm);
        ...
        for (int i = 0; i < d; i++) {
            /* Unpack 3-bit index: lower 2 bits from qs, upper 1 bit from qh */
            uint8_t low2 = (x[block].qs[i / 4] >> ((i % 4) * 2)) & 0x3;
            uint8_t hi1  = (x[block].qh[i / 8] >> (i % 8)) & 0x1;
            uint8_t idx  = low2 | (hi1 << 2);

            /* QJL sign: +1 if bit set, -1 otherwise */
            float sign = (x[block].signs[i / 8] & (1 << (i % 8))) ? 1.0f : -1.0f;

            dst[i] = CENTROIDS_3BIT[idx] * norm + sign * rnorm * inv_sqrt_d;
        }
    }
}
