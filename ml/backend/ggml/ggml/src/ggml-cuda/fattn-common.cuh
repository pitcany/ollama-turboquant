...
// Each thread processes contiguous K elements matching Q_reg layout (same as f16 pattern).
// Optimizations vs scalar dequant-per-element:
//   1. Bulk-load packed qs[] bytes as uint32_t/uint16_t (coalesced, 1 load per 4-8 elements)
//   2. Pre-scale centroids by block norm (N_centroids muls replaces 2*D per-element muls)
//   3. Centroids copied to registers (avoids __constant__ memory warp serialization)

template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo2(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo2_0 * K_blk = (const block_turbo2_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo3(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo3_0 * K_blk = (const block_turbo3_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo4(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo4_0 * K_blk = (const block_turbo4_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
// ---- Turbo2_0_64 KQ inner product (block_size = 64, head_dim = 64 only) ----
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo2_0_64(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo2_0_64 * K_blk = (const block_turbo2_0_64 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
// ---- Turbo3_0_64 KQ inner product (block_size = 64, head_dim = 64 only) ----
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo3_0_64(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo3_0_64 * K_blk = (const block_turbo3_0_64 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
// ---- Turbo4_0_64 KQ inner product (block_size = 64, head_dim = 64 only) ----
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo4_0_64(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo4_0_64 * K_blk = (const block_turbo4_0_64 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
#pragma unroll
    for (int k0 = 0; k0 < D/2; k0 += nthreads*cpy_ne) {
        ...
        const int elem0 = base * 2;
        const int blk   = elem0 / QK_TURBO_64;
        const int qs_byte = (elem0 % QK_TURBO_64) / 2;  // 2 elements per byte for 4-bit
...
// ---- Turbo5 KQ inner product (vectorised: 8 elements per loop iter) ----
//
// With cpy_ne=4 the loop processes 8 elements per iteration, and the
// per-thread base index always advances by a multiple of 8, so the
// 5-bit-packed bit-offset always advances by 40 bits = 5 bytes - byte-
// aligned. We fetch 5 bytes (uint32_t + uint8_t) into a uint64_t and
// decode all 8 indices via shift/mask in registers, replacing 8 scalar
// dequant_element calls with a single bulk fetch.
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turbo5(
    const char * __restrict__ K_c, const void * __restrict__ Q_v,
    const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turbo5_0 * K_blk = (const block_turbo5_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
#pragma unroll
        for (int k0 = 0; k0 < D/2; k0 += nthreads*cpy_ne) {
            ...
            const int elem0 = base * 2;
            const int blk   = elem0 / QK_TURBO5;
            const int elem0_in_blk = elem0 % QK_TURBO5;
...
constexpr __device__ vec_dot_KQ_t get_vec_dot_KQ() {
    if constexpr (type_K == GGML_TYPE_F16) {
        ...
        return vec_dot_fattn_vec_KQ_turbo2<D, nthreads>;
    ...
    } else if constexpr (type_K == GGML_TYPE_TURBO4_0_64) {
        return vec_dot_fattn_vec_KQ_turbo4_0_64<D, nthreads>;
    } else if constexpr (type_K == GGML_TYPE_TURBO2_0_64) {
        return vec_dot_fattn_vec_KQ_turbo2_0_64<D, nthreads>;
    } else if constexpr (type_K == GGML_TYPE_TURBO3_0_64) {
        return vec_dot_fattn_vec_KQ_turbo3_0_64<D, nthreads>;
    } else if constexpr (type_K == GGML_TYPE_TURBO5_0) {
        return vec_dot_fattn_vec_KQ_turbo5<D, nthreads>;
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turbo2(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turbo2_0 * x = (const block_turbo2_0 *) vx;

    const int64_t ib = i0 / QK_TURBO2;
    const int     j  = i0 % QK_TURBO2;
    const float norm = __half2float(x[ib].norm);

#pragma unroll
    for (int l = 0; l < ne; ++l) {
        const float val = turbo2_dequant_element(&x[ib], j + l, norm);
        if constexpr (std::is_same_v<T, half>) {
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turbo3(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turbo3_0 * x = (const block_turbo3_0 *) vx;

    const int64_t ib = i0 / QK_TURBO3;
    const int     j  = i0 % QK_TURBO3;
    const float norm = __half2float(x[ib].norm);

#pragma unroll
    for (int l = 0; l < ne; ++l) {
        const float val = turbo3_dequant_element(&x[ib], j + l, norm);
        if constexpr (std::is_same_v<T, half>) {
            ((half *) dst)[l] = __float2half(val);
        } else {
            ((float *) dst)[l] = val;
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turbo4(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turbo4_0 * x = (const block_turbo4_0 *) vx;

    const int64_t ib = i0 / QK_TURBO4;
    const int     j  = i0 % QK_TURBO4;
    const float norm = __half2float(x[ib].norm);

#pragma unroll
    for (int l = 0; l < ne; ++l) {
        const float val = turbo4_dequant_element(&x[ib], j + l, norm);
        if constexpr (std::is_same_v<T, half>) {
            ((half *) dst)[l] = __float2half(val);
        } else {
            ((float *) dst)[l] = val;
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turbo2_0_64(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turbo2_0_64 * x = (const block_turbo2_0_64 *) vx;

    const int64_t ib = i0 / QK_TURBO_64;
    const int     j  = i0 % QK_TURBO_64;
    const float norm = __half2float(x[ib].norm);

#pragma unroll
    for (int l = 0; l < ne; ++l) {
        const float val = turbo2_0_64_dequant_element(&x[ib], j + l, norm);
        if constexpr (std::is_same_v<T, half>) {
            ((half *) dst)[l] = __float2half(val);
        } else {
            ((float *) dst)[l] = val;
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turbo3_0_64(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
...
#pragma unroll
    for (int l = 0; l < ne; ++l) {
        const float val = turbo3_0_64_dequant_element(&x[ib], j + l, norm);
        if constexpr (std::is_same_v<T, half>) {
            ((half *) dst)[l] = __float2half(val);
        } else {
            ((float *) dst)[l] = val;
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turbo4_0_64(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
...
#pragma unroll
    for (int l = 0; l < ne; ++l) {
        const float val = turbo4_0_64_dequant_element(&x[ib], j + l, norm);
        if constexpr (std::is_same_v<T, half>) {
            ((half *) dst)[l] = __float2half(val);
        } else {
            ((float *) dst)[l] = val;
...
constexpr __device__ dequantize_V_t get_dequantize_V() {
    if constexpr (type_V == GGML_TYPE_F16) {
        ...
        return dequantize_V_turbo2<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TURBO3_0) {
        return dequantize_V_turbo3<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TURBO4_0) {
        return dequantize_V_turbo4<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TURBO4_0_64) {
        return dequantize_V_turbo4_0_64<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TURBO2_0_64) {
        return dequantize_V_turbo2_0_64<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TURBO3_0_64) {
        return dequantize_V_turbo3_0_64<T, ne>;
    } else {
