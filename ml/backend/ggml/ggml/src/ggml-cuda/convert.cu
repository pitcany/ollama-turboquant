...
template <typename dst_t>
static __global__ void dequantize_block_turbo4_0_64(const void * __restrict__ vx, dst_t * __restrict__ y, const int64_t k) {
    const int64_t i = (int64_t)blockDim.x * blockIdx.x + threadIdx.x;
    if (i >= k) return;

    const block_turbo4_0_64 * x = (const block_turbo4_0_64 *) vx;
    const int64_t ib = i / QK_TURBO_64;
    const int     j  = i % QK_TURBO_64;
    const float norm = __half2float(x[ib].norm);
    y[i] = ggml_cuda_cast<dst_t>(turbo4_0_64_dequant_element(&x[ib], j, norm));
}

template <typename dst_t>
static __global__ void dequantize_block_turbo2_0_64(const void * __restrict__ vx, dst_t * __restrict__ y, const int64_t k) {
    const int64_t i = (int64_t)blockDim.x * blockIdx.x + threadIdx.x;
    if (i >= k) return;
    ...
    const int     j  = i % QK_TURBO_64;
    const float norm = __half2float(x[ib].norm);
    y[i] = ggml_cuda_cast<dst_t>(turbo2_0_64_dequant_element(&x[ib], j, norm));
}

template <typename dst_t>
static __global__ void dequantize_block_turbo3_0_64(const void * __restrict__ vx, dst_t * __restrict__ y, const int64_t k) {
    const int64_t i = (int64_t)blockDim.x * blockIdx.x + threadIdx.x;
    if (i >= k) return;
    ...
    const int     j  = i % QK_TURBO_64;
    const float norm = __half2float(x[ib].norm);
    y[i] = ggml_cuda_cast<dst_t>(turbo3_0_64_dequant_element(&x[ib], j, norm));
}
...
template <typename dst_t>
static void dequantize_row_turbo4_0_64_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_turbo4_0_64<<<num_blocks, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template <typename dst_t>
static void dequantize_row_turbo2_0_64_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_turbo2_0_64<<<num_blocks, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}

template <typename dst_t>
static void dequantize_row_turbo3_0_64_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    const int num_blocks = (k + CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / CUDA_DEQUANTIZE_BLOCK_SIZE;
    dequantize_block_turbo3_0_64<<<num_blocks, CUDA_DEQUANTIZE_BLOCK_SIZE, 0, stream>>>(vx, y, k);
}
...
to_fp16_cuda_t ggml_get_to_fp16_cuda(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_TURBO4_0_64:
            return dequantize_row_turbo4_0_64_cuda;
        case GGML_TYPE_TURBO2_0_64:
            return dequantize_row_turbo2_0_64_cuda;
        case GGML_TYPE_TURBO3_0_64:
            return dequantize_row_turbo3_0_64_cuda;
