...
// ---- TurboQuant dequantize stubs (ib/iqs interface for dequantize_block template) ----
...
static __device__ __forceinline__ void dequantize_turbo2_0(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    ...
    v.x = turbo2_dequant_element(&x[ib], 2*iqs + 0, norm);
    v.y = turbo2_dequant_element(&x[ib], 2*iqs + 1, norm);
}
...
static __device__ __forceinline__ void dequantize_turbo3_0(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    ...
    const float norm = __half2float(x[ib].norm);
    v.x = turbo3_dequant_element(&x[ib], 2*iqs + 0, norm);
    v.y = turbo3_dequant_element(&x[ib], 2*iqs + 1, norm);
}
